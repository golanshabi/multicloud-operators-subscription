...
import (
	...
	plrapis "github.com/open-cluster-management/multicloud-operators-placementrule/pkg/apis/apps/v1"
...
func getDecisionsFromPlacementRef(pref *corev1.ObjectReference, namespace string, kubeClient client.Client) ([]string, error) {
	...
	var placement plrapis.PlacementRule
	err = kubeClient.Get(context.TODO(), client.ObjectKey{Namespace: namespace, Name: pref.Name}, &placement)
	if err != nil {
		return nil, err
	}

	klog.Info("----------------------------------------------------------------------------------------------------")
	klog.Infof("%v", placement.Status.Decisions)
	klog.Info("----------------------------------------------------------------------------------------------------")
